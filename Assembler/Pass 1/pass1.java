...
class pass1 {
    ...
    public static void main(String args[]) throws Exception {
                        ...
                        if (x[0] == '=') {
                            ...
                            if (!searchsym(y) && isflag == 0 && stp == 0) {
                                ...
                                if (splitted.hasMoreTokens())
                                    ans = "";
                            } else if (!searchsym(y) && isflag == 1 && stp == 0) {// if instruction has passed on the line
                                                                                     // then only add the symbol not the address
                                sym[symcounter++][0] = y;
                                ans += "(S," + Integer.toString(indexsym(y) + 1) + ")";
        ...
        String ans1 = ""; // SYMBOL TABLE
            ...
            // Write the buffers to their respective files
            writer.write(buffer);
            writer1.write(buffer1);
            writer2.write(buffer2);

            // Close all the file readers and writers
            reader.close();
            writer.close();
            writer1.close();
            writer2.close();
        ...
        // Helper methods to search for mnemonics and tables
